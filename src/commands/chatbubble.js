...
const command = new Command(
    ...
    async function execute(message, args, fromInteraction) {
        ...
        const supportedFiletypes = ["jpeg", "jpg", "png", "webp", "gif", "tiff", "avif", "heif"]
        const extension = args.get("image").name.split(".").pop()
        if (!supportedFiletypes.includes(extension)) {
            action.reply(message, { 
                content: "image format *probably* not supported, this is based off a list of sure supported ones" 
            })
            return
        }
        ...
        const fullHeight = outputImgMetadata.pageHeight ?? outputImgMetadata.height
        const tailHeight = Math.round(fullHeight / 4)
        tailImg = tailImg
            .resize({ width: outputImgMetadata.width, height: tailHeight, fit: "fill" })
            .extend({ bottom: fullHeight - tailHeight, background: { r: 0, g: 0, b: 0, alpha: 0 }})
        
        tailImg.toFile("fuck.png", (err, inf) => {})
        console.log(`fullHeight: ${fullHeight}, tailHeight: ${tailHeight}, reportedHeight: ${await tailImg.metadata().then((metadata) => metadata.height)}`)

        outputImg = await outputImg
            .composite([ { input: await tailImg.toBuffer(), blend: "dest-out", gravity: "north", tile: true } ])
            .toFormat("gif")
            .toBuffer()
