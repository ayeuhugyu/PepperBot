...
const command = new Command(
    {
        name: 'head',
        description: 'outputs the first few lines or bytes of the piped text',
        long_description: 'outputs the first few lines or bytes of the piped text. \nthis command is purely for piping to, and will not work on its own.',
        tags: [CommandTag.Utility, CommandTag.TextPipable],
        options: [
            new CommandOption({
                name: 'options',
                description: 'arguments for head command (e.g., -n 10 or -c 20)',
                long_description: 'arguments to be used, e.g. -n 10 to output the first 10 lines or -c 20 to output the first 20 bytes',
        ...
        example_usage: "p/git log | head -n 10",
    },
    ...
    async function execute({ invoker, piped_data, guild_config, args, will_be_piped }) {
        ...
        if (piped_data.data.input_text) {
            ...
            if (linesArg || !bytesArg) {
                ...
                const output = lines.slice(0, numLines);
                output.unshift(`head: \`\`\`\n`);
                output.push("```");
            ...
            await action.reply(invoker, { content: "no text found to head", ephemeral: guild_config.other.use_ephemeral_replies });
            return new CommandResponse({
                error: true,
                message: "no text found to head",
            });
        }
    }
);
