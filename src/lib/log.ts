...
function log(level: Level, ...message: unknown[]) {
    ...
    if (!nonGlobalLevels.includes(level) || (/*stringToBool(process.env.IS_DEV || "")*/ process.argv.includes("--logdebug") && !neverGlobalLevels.includes(level))) process.stdout.write(stdoutString);
